...
class TableMultiPlayerDeckViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if cell.name.text == "Vlestid"{
            if self.isUnlockedVlestid == true {
                cell.isUserInteractionEnabled = true
            }
            else {
                cell.isUserInteractionEnabled = false
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if let destination = segue.destination as? Fight2ViewController{
            destination.userSyncTableData = doc[(self.tableView.indexPathForSelectedRow?.row)!]
        }
    }
    
    private func verifyLBathUser(){
        ...
        do{
            ...
            if(results.count > 0){
                for r in results as! [NSManagedObject]{
                    if r.value(forKey: "lbathWin") as? Bool == true{
    ...
    private func verifyVlestidUser(){
        //var isUnlocked : Bool = false
        let appDelegate = UIApplication.shared.delegate as! AppDelegate
        let context = appDelegate.persistentContainer.viewContext
        let request2 = NSFetchRequest<NSFetchRequestResult>(entityName: "Fights")
        request2.returnsObjectsAsFaults = false
        do{
            let results = try context.fetch(request2)
            if(results.count > 0){
                for r in results as! [NSManagedObject]{
                    if r.value(forKey: "vlestidWin") as? Bool == true{
    ...
    private func verifyPAJUser(){
        ...
        let appDelegate = UIApplication.shared.delegate as! AppDelegate
        let context = appDelegate.persistentContainer.viewContext
        let request3 = NSFetchRequest<NSFetchRequestResult>(entityName: "Cards")
        request3.returnsObjectsAsFaults = false
        do{
            let results = try context.fetch(request3)
            if(results.count > 0){
                for r in results as! [NSManagedObject]{
